...
class CipherRaceCog(commands.Cog, name="Cipher Race"):
    ...
    async def startrace(self, ctx, sheet: str = cipher_race_constants.HP):
        ...
        ) = cipher_race_utils.create_code_embed(
        ...
        time = Timer(
            cipher_race_constants.BREAK_TIME,
    ...
    async def practice(
        ...
        if code not in cipher_race_constants.CIPHERS:
            ...
            if code is not None:
                embed.add_field(
                    name=f"{cipher_race_constants.CODE.capitalize()} not found!",
            ...
            if (
                code.lower()
                in self.sheet_map[sheet][cipher_race_constants.CODE]
                .value_counts()
                .index
            ):
                used_cipher = code.lower()
                proposal_row = self.sheet_map[sheet][
                    self.sheet_map[sheet][cipher_race_constants.CODE] == used_cipher
                ].sample()
    ...
    async def answer(self, ctx, *args):
        ...
        result = cipher_race_utils.get_answer_result(
            user_answer, self.current_races[channel][cipher_race_constants.ANSWERS]
        )
    ...
    async def send_times_up_message(self, ctx, channel, level):
        ...
        if (
            len(self.current_races[channel][cipher_race_constants.ANSWERS]) < 1
            or self.current_races[channel][cipher_race_constants.LEVEL] != level
        ):
